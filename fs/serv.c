...
#define MAXOPEN 1024
#define FILEVA 0xD0000000 // = FDTABLE
...
int openfile_lookup(envid_t envid, uint32_t fileid, struct OpenFile **po)
{
	...
	o = &opentab[fileid % MAXOPEN]; // to avoid array overflow
...
int serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((errno = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return errno;

	ssize_t r;
	r = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset);
	// req_n may be greater than PGSIZE?
