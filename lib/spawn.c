			...
			int fd, size_t filesz, off_t fileoffset, int perm)
{
	...
	// cprintf("%s:%d map_segment %x+%x\n", __FILE__, __LINE__, va, memsz);
	...
	for (i = 0; i < memsz; i += PGSIZE)
	{
		...
		else
		{
			...
			// cprintf("%s:%d map_segment: sys_page_map(0, 0x%x, %x, 0x%x, %d)\n",
			// 		__FILE__, __LINE__, UTEMP, child, (void *)(va + i), perm);
			// {
			// 	char *it;
			// 	cprintf("it = 0x%x, va = 0x%x, i = 0x%x\n", UTEMP, va, i);
			// }
			if ((r = sys_page_map(0, UTEMP, child, (void *)(va + i), perm)) < 0)
				panic("spawn: sys_page_map data: %e", r);
			// cprintf("%s:%d map_segment: 6\n", __FILE__, __LINE__);
			sys_page_unmap(0, UTEMP);
			// cprintf("%s:%d map_segment: 7\n", __FILE__, __LINE__);
		}
	}
	return 0;
}
