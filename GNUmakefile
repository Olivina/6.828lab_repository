...
KERN_CFLAGS := $(CFLAGS) -DJOS_KERNEL -gstabs #"-DTEST=`case $* in *_*) echo $*;; *) echo user_$*;; esac`"
USER_CFLAGS := $(CFLAGS) -DJOS_USER -gstabs #"-DTEST=`case $* in *_*) echo $*;; *) echo user_$*;; esac`"
...
QEMUOPTS = -drive file=$(OBJDIR)/kern/kernel.img,index=0,media=disk,format=raw -serial mon:stdio -gdb tcp::$(GDBPORT) -d int
#QEMUOPTS = -drive file=$(OBJDIR)/kern/kernel.img,index=0,media=disk,format=raw -serial mon:stdio --enable-kvm
